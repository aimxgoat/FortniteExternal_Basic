...
auto actors_t::execute_loop( ) -> void {
    ...
    for ( int i = 0; i < pointers.player_array_size; ++i ) {
        uintptr_t player_state = mem::Read1<uintptr_t>( pointers.player_array + ( i * sizeof( uintptr_t ) ));
        uintptr_t current_actor = mem::Read1<uintptr_t>( player_state + offsets.pawn_private );
        uintptr_t actors_mesh = mem::Read1<uintptr_t>( current_actor + offsets.mesh );
        uintptr_t otherplaystate = mem::Read1<uintptr_t>( current_actor + offsets.player_state );
       ...
       /* auto team_index = mem::Read1<int>( otherplaystate + offsets.team_index );
        ...
        if ( target_entity && cfg_t::aimbot::aimbot_t ) {
            auto closest_entity = mem::Read1<uint64_t>( target_entity + offsets.mesh );
