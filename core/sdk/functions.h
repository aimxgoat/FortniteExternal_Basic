...
public:
	...
	{
		uintptr_t bone_array = mem::Read1<uintptr_t>( skeletal_mesh + 0x5E8);

		if ( bone_array == NULL ) bone_array = mem::Read1<uintptr_t>( skeletal_mesh + 0x5E8 + 0x48) * 0x10;

		ftransform bone = mem::Read1<ftransform>(bone_array + ( bone_index * 0x60 ) );

		ftransform component_to_world = mem::Read1<ftransform>( skeletal_mesh + offsets.component_to_world );
	...
	void get_view_state()
	{
		uintptr_t view_matrix = mem::Read1<uintptr_t>(pointers.local_player + 0xd0);

		view_state = mem::Read1<uintptr_t>(view_matrix + 0x8);
	}
	...
	auto initialize_camera( ) -> camera_t {
		...
		camera_t camera;
		auto m_projection = mem::Read1<f_matrix>(view_state + 0x930 );
		...
		float field_of_view = atanf(1 / mem::Read1<double>(view_state + 0x730 ) ) * 2;
	...
	bool is_entity_visible(uintptr_t mesh) {
		auto Seconds = mem::Read1<double>(pointers.gworld + 0x198);
		auto LastRenderTime = mem::Read1<float>(mesh + 0x32c);
...
public:
	...
	auto decrypt_platform( uintptr_t platforms ) {
		uintptr_t platform = mem::Read1<uintptr_t>( platforms + 0x430 );
		wchar_t platform_bit[64] = { 0 };

		mem::read_physical( PVOID( platform ), reinterpret_cast<uint8_t*>( platform_bit ), sizeof( platform_bit ) );
	...
	static std::string decrypt_username( uintptr_t player_state ) {
		...
		uintptr_t name_structure = mem::Read1<uintptr_t>( player_state + 0xb08 );

		if (mem::is_valid( name_structure ) ) {
			name_length = mem::Read1<int>(name_structure + 0x10);
			if (name_length <= 0)
				return "BOT/Ai";

			name_buffer = new char16_t[name_length];
			uintptr_t encrypted_buffer = mem::Read1<uintptr_t>( name_structure + 0x8 );
			if (mem::is_valid( encrypted_buffer ) ) {
				if (mem::read_array2( encrypted_buffer, name_buffer, name_length ) ) {
